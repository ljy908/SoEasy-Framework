...
int txUart(int gpio_fd, int gpioNumber) {
	...
	char gpioLocation[255]; // Store the value read from DB.
	char uart[100];
	...
	if (loopCounter == 0) {
		...
		while (1) {
			config_count[i++] = con_data[j]; // The comparison condition value is input.
	...
	while (1) {
		if (dataCondition == 1) {
			if (data[i] == '=') // when the last character
					{
